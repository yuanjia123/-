...
if __name__ == '__main__':
    ...
    id = [4298989816221761, 4298982337445859, 4298996928897636, 4299418075146118, 4299406599766183,
          ...
          4298989430127358]
    max_id = [4299701576388896, 4299307195064641, 4307760998861888, 4299445731873795, 4307771342057892,
    ...
    for i in range(1, len(id)):
        ...
        for j in range(1,page):  #拿到页面 数以后进行遍历。
            ...
            for m in range(1,len(id_list)):
                private_url = oneself.format(id_list[m])      #取每一个用户id。 然后进行拼接。
                print("详细页面的url", private_url)    #生成每一个用户的url.
                #a.location_parse(session.get(private_url, headers= a.headers, verify=False))      #连接上登录用户class返回的 session 去session登录。
